// ──────────────────────────────────────────────────────────────────────
// ARCHIVED: V1 profile prompt assembly path
// This was the original 5-layer prompt assembly used in buildProfilePrompt()
// before V2 switched to the self-contained prompt-v2.txt template.
// The V1 path loaded the Geoffrey Block, real exemplars (Newmark, Bahat,
// McGlinchey), and a contamination fence separately.
// ──────────────────────────────────────────────────────────────────────

import type { LinkedInData } from '../../prompts/extraction-prompt';

/**
 * V1 profile prompt assembly — the else branch from buildProfilePrompt().
 * Accepts pre-loaded geoffreyBlock and exemplars (real-person profiles).
 */
export function buildProfilePromptV1(
  donorName: string,
