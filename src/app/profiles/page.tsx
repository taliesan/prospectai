    ...
    <div className="min-h-screen bg-brand-off-white">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <h1 className="font-serif text-3xl text-brand-black">Your Profiles</h1>
          <a
            href="/"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <h2 className="font-serif text-2xl text-brand-black mb-2">No profiles yet</h2>
            <p className="text-brand-mid-gray mb-6">Generate your first donor profile to get started.</p>
            <a
              href="/"
          ...
          <div className="space-y-6">
            {Object.entries(grouped).map(([donorName, donorProfiles]) => (
              <div key={donorName} className="bg-white rounded-2xl border border-brand-light-gray overflow-hidden"
                   style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
                <div className="px-6 py-4 border-b border-brand-light-gray flex items-center justify-between">
                  <a href={`/profile/${donorProfiles[0].id}`} className="font-serif text-xl text-brand-black hover:text-purple-800 transition-colors">
                    {donorName}
                  </a>
                  <span className="text-xs text-brand-mid-gray">{donorProfiles.length} version{donorProfiles.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="divide-y divide-brand-light-gray">
                  {donorProfiles.map((profile) => {
                    ...
                    return (
                      <div key={profile.id} className="px-6 py-4 flex items-center justify-between hover:bg-brand-off-white/50 transition-colors">
                        <div className="flex items-center gap-4">
                          <a href={`/profile/${profile.id}`} className="text-sm font-medium text-brand-black hover:text-brand-purple transition-colors">
                            {date}
                          </a>
                          {profile.sourceCount && (
                            <span className="text-xs text-brand-mid-gray">{profile.sourceCount} sources</span>
                          )}
                          {(() => {
                            const avg = getAvgConfidence(profile.confidenceScores);
                            return avg !== null ? (
                              <span className="text-xs text-brand-mid-gray" title="Average confidence score">
                                {avg}/10
                              </span>
                            ) : null;
                          })()}
                          {profile.pipelineVersion && (
                            <span className="text-[10px] font-semibold tracking-[1px] uppercase text-brand-mid-gray border border-brand-light-gray rounded px-1.5 py-0.5">
                              {profile.pipelineVersion}
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-3">
                          <a
                            ...
                            className="px-3 py-1.5 text-xs font-medium rounded bg-brand-off-white hover:bg-brand-light-gray text-brand-warm-gray transition-colors"
                          >
                            View
                          </a>
                          <button
                            onClick={() => deleteProfile(profile.id)}
                            className="px-3 py-1.5 text-xs font-medium rounded text-brand-mid-gray hover:text-brand-red hover:bg-red-50 transition-colors"
                          >
                            Delete
                          </button>
                        </div>
